	...
	for(int j=0; j<ny; ++j)
		for(int i=0; i<nx; ++i){
			...
			for(int k=0; k<Q; ++k){
				...
				// cout << k << ' ' << i << ' ' << j << " B: " << f[k][i][j] << '\n';
				f[k][i][j] = (1.-omega)*f[k][i][j] + omega*feq[k][i][j];
				// cout << k << ' ' << i << ' ' << j << " A: " << f[k][i][j] << '\n';
...
vector<vector<vector<double>>> ruv(
	...
	for(int i=0; i<nx; ++i)
		rho[i][ny-1] = f[0][i][ny-1] + f[1][i][ny-1] + f[3][i][ny-1] + 2.*(f[2][i][ny-1] + f[5][i][ny-1] + f[6][i][ny-1]);
...
int main(){
	...
	if(init)
		initialize(nx, ny, u, v, cx, cy, f, feq, rho, w, Q);
	...
	vector<double> x;  // x coordinates of nodes
	vector<double> y;
