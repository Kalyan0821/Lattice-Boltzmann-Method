...
set<pair<int, int>> findInternalPoints(const set<pair<int, int>>& boundary_points, int N){

	set<pair<int, int>> internal_points;
	for(int j=-(N-1); j<N; ++j)
		for(int i=0; i<N; ++i){

			if(boundary_points.find({i, j}) != end(boundary_points))
				break;
			else{
...
void translate(
	...
	boundary_points = addCenterToSet(boundary_points, i0, j0);
...
Boundary findStairBoundary(double r, double dx, int i0, int j0){
	...
	while(j <= i){
		if(rError(i, j, dx, r) <= rError(i-1, j, dx, r)){
        	...
        	boundary_points.insert({i-1, j});
	        type_to_points["w"].insert({i-1, j});
	        type_to_points["s"].insert({i, j-1});
	         
	        boundary_points.insert({i-1, j-1});
	...
	completeBoundary(type_to_points, boundary_points, octant_to_type);
	internal_points = findInternalPoints(boundary_points, N);
	translate(type_to_points, boundary_points, internal_points, i0, j0);

	return {type_to_points, boundary_points, internal_points};
}

void writeToFile(const set<pair<int, int>> boundary_points, string name){
...
// 	Boundary information = findStairBoundary(0.05, 1./500, 100, 100);
// 	writeToFile(information.boundary_points, "./output/boundary_points.txt");
